...
class AuthConfig:
    ...
    # 비밀번호 정책
    MIN_PASSWORD_LENGTH: int = 12  # 보안 강화: 8 -> 12자
    ...
    def SECRET_KEY(self) -> str:
        ...
        if not settings.jwt_secret_key or len(settings.jwt_secret_key) < 32:
            raise ValueError(
                "JWT_SECRET_KEY가 설정되지 않았거나 너무 짧습니다. "
...
def generate_backup_codes(count: int = 10) -> Tuple[list, list]:
    ...
    Returns:
        Tuple[list, list]: (평문 코드 목록, 해시된 코드 목록)
    """
    import json
    plain_codes = []
    hashed_codes = []
...
def verify_backup_code(code: str, hashed_codes_json: str) -> Tuple[bool, Optional[str]]:
    ...
    Args:
        code: 입력된 백업 코드
        hashed_codes_json: JSON 형태의 해시된 백업 코드 배열

    Returns:
        Tuple[bool, Optional[str]]: (유효 여부, 사용된 코드 제거 후 JSON)
    """
    import json
...
class AuthService:
    ...
    async def login(
        ...
        if admin.is_2fa_enabled:
            ...
            # TOTP 코드 먼저 확인
            totp_valid = verify_totp(admin.totp_secret, request.totp_code)

            if not totp_valid:
                # 백업 코드로 시도
                backup_valid, remaining_codes = verify_backup_code(
                    request.totp_code, admin.totp_backup_codes
                )

                if backup_valid:
                    # 백업 코드 사용됨 - 남은 코드 업데이트
                    admin.totp_backup_codes = remaining_codes

                    # 백업 코드 사용 감사 로그
                    backup_audit_log = AuditLog(
                        admin_id=admin.admin_id,
                        action="backup_code_used",
                        details="2FA backup code used for login",
                        ip_address=ip_address,
                        user_agent=user_agent
                    )
                    session.add(backup_audit_log)
                else:
    ...
    async def create_admin(
        ...
        # 중복 확인 - username만 필수, email은 있을 때만 체크
        conditions = [Admin.username == request.username]
        if request.email:
            conditions.append(Admin.email == request.email)
        
        result = await session.execute(
            select(Admin).where(or_(*conditions))
        )
